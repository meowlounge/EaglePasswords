...
export const isTokenPayloadValid = (
	...
	try {
		...
		const matchesId = matchCriteria.id
			? payload.id === matchCriteria.id
			: true;
 ...
 */
export const getValidatedUserIdFromToken = (userId: string): string | null => {
...
export const enableTwoFactorAuth = async (): Promise<{
	...
	const data = await apiRequest<{ otpauthUrl: string }>(
		`/api/twofactor/enable/${userId}`,
		{
			method: 'POST',
			headers: {
				Authorization: `Bearer ${token}`,
