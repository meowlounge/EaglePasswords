          ...
          if (importFormat === 'csv') {
               Papa.parse(file, {
                    complete: async (result) => {
                         ...
                         try {
                              ...
                              if (skipped.length > 0) {
                                   setError(
                                        `The following rows were skipped: ${skipped.join(', ')}`
                                   );
                              }
                         } catch (error) {
                              setError(
                                   'Failed to upload passwords. Please try again.'
                              );
                              console.error(
                                   'Error during password upload:',
                                   error
                              );
                         } finally {
               ...
               reader.onload = async () => {
                    try {
                         ...
                         setError(
                              'Failed to parse JSON file. Ensure the file is in a valid format.'
                         );
