...
export const getPasswordStrength = (
	...
	if (score >= 50)
		return { label: 'Fair', variant: 'warning', percentage: 50 };
		...
		>
			...
			<div className='p-4'>
				<div className='flex items-center justify-between mb-4'>
					...
					>
						<button
							onClick={() => handleFavorite(password)}
							className={`p-2 rounded-md transition-all 
                                   ${
										password.isFavorite
											? 'bg-yellow-100 dark:bg-yellow-400/10 text-yellow-400 hover:bg-yellow-200 dark:hover:bg-yellow-300/20'
											: 'text-neutral-500 hover:text-neutral-800 hover:bg-yellow-200 dark:text-neutral-400 dark:hover:text-yellow-400 dark:hover:bg-yellow-300/20'
									}
                                   `}
						>
							<StarIcon
								size={14}
								className={`${password.isFavorite ? 'text-yellow-400' : 'text-neutral-400'} transition-colors`}
							/>
						</button>
						<button
