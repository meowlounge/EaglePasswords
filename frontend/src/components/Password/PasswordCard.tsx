...
export const getPasswordStrength = (password: string): {
    ...
    if (score >= 100) return { label: 'Strong', variant: 'success', percentage: 100 };
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between py-2 px-3 bg-neutral-200 rounded-md group dark:bg-neutral-800/70">
                        <div className="min-w-0 flex-1">
                            <div className="text-xs text-neutral-600 mb-1 dark:text-neutral-400">Username</div>
                        ...
                        <div className="flex items-center gap-1 ml-4">
                            <button
                                onClick={() => setshowUsername({ ...showUsername, [password.id]: !showUsername[password.id] })}
                            ...
                            >
                                {showUsername[password.id] ? <EyeOff size={14} /> : <Eye size={14} />}
                            </button>
                            <button
                                onClick={() => copyToClipboard('username')}
                                className="p-1.5 text-neutral-600 hover:text-neutral-800 hover:bg-neutral-300 rounded-xs transition-all dark:text-neutral-400 dark:hover:text-neutral-100 dark:hover:bg-neutral-700/50"
                            >
                                {copied === 'username' ? <CheckCircle size={14} className="text-green-500" /> : <Copy size={14} />}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between py-2 px-3 bg-neutral-200 rounded-md group dark:bg-neutral-800/70">
                        <div className="min-w-0 flex-1">
                            <div className="text-xs text-neutral-600 mb-1 dark:text-neutral-400">Password</div>
                            <div className="font-mono text-sm text-neutral-700 truncate dark:text-neutral-100">
                        ...
                        <div className="flex items-center gap-1 ml-4">
                            ...
                            <button
                                ...
                                className="p-1.5 text-neutral-600 hover:text-neutral-800 hover:bg-neutral-300 rounded-xs transition-all dark:text-neutral-400 dark:hover:text-neutral-100 dark:hover:bg-neutral-700/50"
                            >
                                {copied === 'password' ? <CheckCircle size={14} className="text-green-500" /> : <Copy size={14} />}
                            </button>
                        </div>
                    </div>
                </div>
